...
exports.smtpRelayExecutor = exports.SMTPRelayExecutor = void 0;
exports.setSMTPService = setSMTPService;
const workflow_1 = require("@metabuilder/workflow");
const nodemailer_1 = __importDefault(require("nodemailer"));
class SMTPRelayExecutor {
    constructor() {
        this.nodeType = 'smtp-relay-send';
    ...
    async execute(node, context, state) {
        ...
        try {
            ...
            const { to, cc, bcc, from, subject, body, template, data, attachments, retryAttempts = 3 } = node.parameters;
            const resolvedTo = (0, workflow_1.interpolateTemplate)(to, {
                context,
                state,
                json: context.triggerData
            });
            const resolvedSubject = (0, workflow_1.interpolateTemplate)(subject, {
                context,
                state,
                json: context.triggerData
            });
            const resolvedFrom = from
                ? (0, workflow_1.interpolateTemplate)(from, {
                    context,
                    state,
                    json: context.triggerData
                })
            ...
            const resolvedCc = cc
                ? (0, workflow_1.interpolateTemplate)(cc, {
                    context,
                    state,
                    json: context.triggerData
                })
            ...
            const resolvedBcc = bcc
                ? (0, workflow_1.interpolateTemplate)(bcc, {
                    context,
                    state,
                    json: context.triggerData
                })
            ...
            if (template) {
                ...
                emailBody = (0, workflow_1.interpolateTemplate)(body, {
                    context,
                    state,
                    json: context.triggerData
                });
            ...
            const messageId = await this._sendWithRetry(tenantTransporter, {
                from: resolvedFrom,
                to: resolvedTo,
                cc: resolvedCc,
                bcc: resolvedBcc,
            ...
            // Log successful send
            console.log(`[SMTP] Message sent: ${messageId} to ${resolvedTo}`, `Subject: ${resolvedSubject}`);
            // Audit log
            await this._auditLog(context, 'email_sent', {
                messageId,
                to: resolvedTo,
                subject: resolvedSubject,
                from: resolvedFrom,
            ...
            // Audit log error
            await this._auditLog(context, 'email_failed', {
                error: errorMessage,
                timestamp: new Date().toISOString()
            }, 'error').catch((err) => {
    ...
    async _getTenantTransporter(tenantId) {
    ...
    async _sendWithRetry(transporter, mailOptions, maxRetries) {
        if (!transporter) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                console.warn(`[SMTP] Attempt ${attempt} failed, retrying in ${delayMs}ms: ${lastError.message}`);
    ...
    async _auditLog(context, action, metadata, status) {
        try {
...
exports.SMTPRelayExecutor = SMTPRelayExecutor;
exports.smtpRelayExecutor = new SMTPRelayExecutor();
/**
 * Email service setter for dependency injection
 * Allows tests or custom configurations
 */
function setSMTPService(executor) {
