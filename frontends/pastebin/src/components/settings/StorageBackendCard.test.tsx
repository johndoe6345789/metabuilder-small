...
describe('StorageBackendCard', () => {
  ...
  describe('DBAL option', () => {
    it('should display DBAL radio option', () => {
      ...
      expect(
        screen.getByTestId('storage-option-dbal')
      ).toBeInTheDocument()
    })
  ...
  describe('state transitions', () => {
    it('should transition from IndexedDB to DBAL', () => {
      ...
      const indexeddbRadio1 = c1.querySelector('input[id="storage-indexeddb"]') as HTMLInputElement
      expect(indexeddbRadio1?.checked).toBe(true)
      ...
      const dbalRadio = c2.querySelector('input[id="storage-dbal"]') as HTMLInputElement
      expect(dbalRadio?.checked).toBe(true)
    })
  })
